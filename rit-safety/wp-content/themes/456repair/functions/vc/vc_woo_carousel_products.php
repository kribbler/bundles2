...
function vc_woo_carousel_products_func( $atts, $content = null ) { // New function parameter $content is added!
   ...
   'carousel_loop' => '',
		...
		global $carousel_products_ID;
		
		$carousel_products_ID = rand();
		...
		if ( $products->have_posts() ) : ?>
			...
			<div class="row">
			<ul class="lpd-products owl-carousel-<?php echo $carousel_products_ID;?>">
		...
		<?php 
		$counter_js = new carousel_products_class();
		
		$counter_js->carousel_products_callback();
		?>
...
add_shortcode( 'vc_woo_carousel_products', 'vc_woo_carousel_products_func' );

class carousel_products_class
{
    protected static $var = '';

    public static function carousel_products_callback(){
	...
	$columns = $shortcode_atts['columns'];
		...
		/* <![CDATA[ */
            jQuery(document).ready(function() {
              jQuery('.owl-carousel-<?php echo $carousel_products_ID;?>').owlCarousel({
...
vc_map(array(
   "name" => __("Products Carousel", GETTEXT_DOMAIN),
   "base" => "vc_woo_carousel_products",
   "class" => "",
   "icon" => "icon-wpb-lpd",
