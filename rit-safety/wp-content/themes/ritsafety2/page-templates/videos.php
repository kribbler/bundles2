...
<?php
	...
	query_posts( 'p='.$_GET['vid'].'&post_type=product ' );   
	while ( have_posts() ) : the_post();
	
			$add_video = get_field('add_video', $_GET['vid']);	
			//var_dump($add_video); var_dump($_GET['vid']);
 			if( $add_video ){ 	
 					 echo '<br><iframe width="560" height="315" src="http://www.youtube.com/embed/'.getYoutubeIdFromUrl($add_video).'" frameborder="0" allowfullscreen></iframe><br><br><br><br><br>';
...
href="<?php echo $permalink; ?>" onclick="goBack()"><img src="<?php bloginfo( 'template_url' ); ?>/images/go-back.jpg"></a>
<script>
function goBack() {
    window.history.back()
}
