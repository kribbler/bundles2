...
//						$args = array( 'numberposts' => $number);
                                                $args = array( 'numberposts' => $number,'post_type'=>news);
						$myposts = get_posts( $args );
						foreach( $myposts as $post ) :	setup_postdata($post); ?>
						<li class="fppostli">
						<a href="<?php the_permalink(); ?>"><?php the_post_thumbnail('fppost'); ?></a> 
                                                <div class="news">
                                                <div class="news-title"><a class="postera" href="<?php the_permalink(); ?>"><?php the_title(); ?></a></div>
                                                 <div class="news-date">-<?php the_time('d/m/Y');?></div></div>
                                                <div class="cnt"><p><?php echo excerpt(10); ?>
                                                <?php
                                                $c=$post->post_content;
												if (preg_match('#<a.*?href=["\'][^"\']+.pdf["\'].*?>.*?</a>#',$c,$r))
													echo '<div class="pdf">',$r[0],'</div>';
                                                ?>
												<?php echo wp_church_continue_reading_link();?></p></div>
...
class netlabs_fpmessage_Widget extends WP_Widget {

    function netlabs_fpmessage_Widget() {
        parent::WP_Widget(false, $name = 'Frontpage message');	
    }
        ...
        ?>
              <?php echo $before_widget; ?>
                  <?php if ( $title )
						...
						<?php
						global $post;
						$args = array( 'numberposts' => $number,'post_type'=>messages);
						$myposts = get_posts( $args );
						foreach( $myposts as $post ) :	setup_postdata($post); ?>
						<li class="fppostli">
						<a href="<?php the_permalink(); ?>"><?php the_post_thumbnail('fppost'); ?></a>  
						<a class="postera" href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
                                                <div class="subheading"><?php echo get_post_meta($post->ID, 'subheading',true); ?></div>
<!--                                                <div class="date"><?php// the_time('d/m/Y') ?></div>-->
                                                <div class="cnt"><p><?php echo excerpt(10).wp_church_continue_reading_link();?></p></div>
<!--						<div class="meta"><?php// wp_church_posted_on(); ?></div> -->
...
add_action('widgets_init', create_function('', 'return register_widget("netlabs_fpmessage_Widget");'));
