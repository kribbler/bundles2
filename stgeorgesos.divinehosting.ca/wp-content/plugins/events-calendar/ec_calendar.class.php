	...
	function displayEventList($num) {
		...
		foreach($events as $event) {
			if ($event->accessLevel == 'public' || $current_user->has_cap($event->accessLevel)) {
				...
				$startDate = date($options['dateFormatWidget'], $timeStp );
				...
				if ($PostID == '')
					...
					$titlinked = '<div class="daydisplayw"><h1>'  . $startDate .'<sup>'.date('S', $timeStp ) .'</sup>'.'</h1></div>' . __(' ', 'events-calendar')
                                                    .'<div class="shortcalentryw"><b>'. $event->eventTitle . '</b><br/>';
					$loc=trim($event->eventLocation);
					if (!empty($loc))
						$titlinked .= '<div class="event-place"><span>Place :</span>'.$event->eventLocation.' </div>';
					if (date ('g:i a',strtotime ($event->eventStartTime))!='12:00 am')
						$titlinked .= '<div class="shortcalentryw_time"><span>Time :</span>'.date ('g:i a',strtotime ($event->eventStartTime)).'</div>';
					$titlinked .= '</div>';
