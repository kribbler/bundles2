...
<?php
			...
			foreach($this->db->getDaysEvents($sqldate) as $e) {
				if (!strpos($e->eventTitle, '|')) { 
					...
					if (($e->accessLevel == 'public') || (current_user_can($e->accessLevel))) {
						...
						if ((!empty($startTime) || !is_null($startTime)) && $startTime<>'12:00 am')
						...
						if ((!empty($endTime) && !empty($startTime) || !is_null($endTime) && !is_null($startTime)) && $endTime<>'12:00 am')
						...
						if ($linkout != '') {
							...
							$titlinked = '<a class="EC-tt-title-link EC-tt-user-link" href="' . addslashes($linkout). '" target="_blank">'.$title.'</a>';
						} elseif ($PostID != '') { // Link to a post when exist
							$titlinked = '<a class="EC-tt-title-link EC-tt-post-link" href="' . addslashes(get_permalink($PostID)) . '">'.$title.'</a>';
						} else {
					...
					if($output != '') {
						...
						$the_js .= 'append(\'<span class="event-block" id="events-calendar-'.$id.'Large">';
						$the_js .= addslashes($titlinked);
    ...
    if($e->accessLevel == 'public' || $current_user->has_cap($e->accessLevel)) {
      ...
      if((!empty($startTime) || !is_null($startTime)) && trim($startTime)!='12:00 am')
      ...
      if((!empty($endTime) && !empty($startTime) || !is_null($endTime) && !is_null($startTime)) && trim($endTime)!='12:00 am')
