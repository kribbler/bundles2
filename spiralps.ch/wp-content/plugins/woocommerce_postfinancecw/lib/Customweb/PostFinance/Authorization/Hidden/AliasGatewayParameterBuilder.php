...
class Customweb_PostFinance_Authorization_Hidden_AliasGatewayParameterBuilder extends Customweb_PostFinance_AbstractParameterBuilder {
...
// 			$this->getAliasManagerParameters(),
		...
		if ($aliasTransaction !== null && $aliasTransaction != 'new') {
			/** @var $aliasTransaction Customweb_PostFinance_Authorization_Transaction */
			...
			if (!isset($params['CARDNO'])) {
				throw new Exception("No CARDNO is defined for the given alias.");
			}
		...
		if ($this->getTransaction()->getAliasTransactionId() !== null) {
			$parameters['ORDERID'] = Customweb_PostFinance_Util::applyOrderSchema(
	...
	protected function getReactionUrlParameters() {
		return array(
			'ACCEPTURL' => $this->getTransactionContext()->getNotificationUrl(),
			'EXCEPTIONURL' => $this->getTransactionContext()->getNotificationUrl()
		);
	}
}