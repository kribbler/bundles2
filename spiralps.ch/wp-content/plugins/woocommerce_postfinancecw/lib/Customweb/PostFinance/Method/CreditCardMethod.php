...
class Customweb_PostFinance_Method_CreditCardMethod extends Customweb_PostFinance_Method_DefaultMethod {
	...
	public function getFormFields(Customweb_Payment_Authorization_IOrderContext $orderContext, $aliasTransaction, $failedTransaction, $authorizationMethod, $isMoto) {
		...
		if (Customweb_PostFinance_Authorization_Hidden_Adapter::AUTHORIZATION_METHOD_NAME == $authorizationMethod ||
				Customweb_PostFinance_Authorization_Server_Adapter::AUTHORIZATION_METHOD_NAME == $authorizationMethod) {
			...
			$errorCardHolder = null;
			$errorCardNumber = null;
			$errorExpiryDate = null;
			$errorCVC = null;
			...
			if ($aliasTransaction !== null && $aliasTransaction !== 'new') {
				...
				if (isset($params['ECOM_CARDINFO_EXPDATE_MONTH'])) {
					$expiryMonth = $params['ECOM_CARDINFO_EXPDATE_MONTH'];
				}
				
				if (isset($params['ECOM_CARDINFO_EXPDATE_YEAR'])) {
					$expiryYear = $params['ECOM_CARDINFO_EXPDATE_YEAR'];
				}
				
				if (isset($params['ED'])) {
					$dates = Customweb_Payment_Util::extractExpiryDate($params['ED']);
					$expiryMonth = $dates['month'];
					$expiryYear = $dates['year'];
				}
				
				$cardHolder = empty($params['CN']) ? $cardHolder : $params['CN'];
				$alias = empty($params['CARDNO']) ? '' : $params['CARDNO'];
			}
			...
			if ($failedTransaction !== null) {
				...
				if ($errorCode !== null) {
					$errorExpiryDate = $this->getExpiryErrorMessage($errorCode);
					$errorCardHolder = $this->getCardHolderErrorMessage($errorCode);
					$errorCardNumber = $this->getCardNumberErrorMessage($errorCode);
					$errorCVC = $this->getCVCErrorMessage($errorCode);
				}
				...
				// The card number is masked, so it has to be reentered by the customer anyway.
				/*if (isset($requestParameters['CARDNO'])) {
					$cardNumber = $requestParameters['CARDNO'];
				}*/
				
				if (isset($requestParameters['CN'])) {
					$cardHolder = $requestParameters['CN'];
				}
				
				if (isset($requestParameters['ECOM_CARDINFO_EXPDATE_MONTH'])) {
					$expiryMonth = $requestParameters['ECOM_CARDINFO_EXPDATE_MONTH'];
				}
				
				if (isset($requestParameters['ECOM_CARDINFO_EXPDATE_YEAR'])) {
					$expiryYear = $requestParameters['ECOM_CARDINFO_EXPDATE_YEAR'];
				}
			...
			$elements[] = Customweb_Form_ElementFactory::getCardHolderElement("CN", $cardHolder, $errorCardHolder);
			$elements[] = Customweb_Form_ElementFactory::getCardNumberElement('CARDNO', $alias, $errorCardNumber, $cardNumber);
			$elements[] = Customweb_Form_ElementFactory::getExpirationElement('ECOM_CARDINFO_EXPDATE_MONTH', 'ECOM_CARDINFO_EXPDATE_YEAR', $expiryMonth, $expiryYear, $errorExpiryDate);
			
			// The CVC is required in any case (also in the MoTo case!)
			$elements[] = Customweb_Form_ElementFactory::getCVCElement('CVC', $errorCVC);
