 ...
 *
 */
final class Customweb_PostFinance_Util {	
	
	private function __construct() {
		// prevent any instantiation of this class	
	}
	
	public static function applyOrderSchema(Customweb_PostFinance_Configuration $config, $transactionId) {
		...
		if(!empty($orderSchema)) {
			...
			if ($totalLength > 30 ) {
				...
				$orderSchema = Customweb_PostFinance_Util::substrUtf8($orderSchema, min($lengthToReduce, strlen($orderSchema)), strlen($orderSchema));
			}
		...
		return Customweb_PostFinance_Util::substrUtf8($id, 0, 30);
	}
	...
	public static function sendRequest($url, $parameters) {
		...
		if($handler->getStatusCode() != 200) {
			throw new Exception('PostFinance Server response is: '
	...
	public static function calculateHash($parameters, $transaction, Customweb_PostFinance_Configuration $configuration) {
		...
		if(strtolower($transaction) == 'in') {
			...
			$allowedParameters = array('AAVADDRESS', 'AAVCHECK', 'AAVZIP', 'ACCEPTANCE', 'ALIAS', 'AMOUNT', 'BIN',
				'BRAND', 'CARDNO', 'CCCTY', 'CN', 'COMPLUS', 'CREATION_STATUS', 'CURRENCY', 'CVCCHECK', 'CVC',
				'DCC_COMMPERCENTAGE', 'DCC_CONVAMOUNT', 'DCC_CONVCCY', 'DCC_EXCHRATE', 'DCC_EXCHRATESOURCE',
				'DCC_EXCHRATETS', 'DCC_INDICATOR', 'DCC_MARGINPERCENTAGE', 'DCC_VALIDHOUS', 'DIGESTCARDNO', 'ECI',
				'ED', 'ENCCARDNO', 'FXAMOUNT', 'FXCURRENCY', 'IP', 'IPCTY', 'NBREMAILUSAGE', 'NBRIPUSAGE',
				'NBRIPUSAGE_ALLTX', 'NBRUSAGE', 'NCERROR', 'NCERRORCVC', 'NCERRORED', 'NCERRORCN', 'NCERRORCARDNO',
				'ORDERID', 'PAYID', 'PM', 'SCO_CATEGORY', 'SCORING',
