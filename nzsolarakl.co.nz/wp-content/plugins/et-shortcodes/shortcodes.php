...
function et_shortcodes_css_and_js(){
	wp_enqueue_style('et-shortcodes-css', ET_SHORTCODES_PLUGIN_URI . '/shortcodes.css', false, ET_SHORTCODES_VERSION, 'all');
	wp_enqueue_script('et-shortcodes-js', ET_SHORTCODES_PLUGIN_URI . '/js/et_shortcodes_frontend.js', array('jquery'), ET_SHORTCODES_VERSION, true);
}
...
function et_imagetab($atts, $content = null) {
	...
	$output = "
		<div{$id} class='et-image{$class}' style='background: url(" . esc_url( $content ) . ") no-repeat; width: {$width}px; height: {$height}px;'><span class='et-image-overlay'> </span></div>";
...
function et_filter_mce_plugin($plugins) {	
	$plugins['et_quicktags'] = ET_SHORTCODES_PLUGIN_URI . '/js/editor_plugin.js';
			...
			form.find('#shortcodes-submit').click(function(){
				...
				if (content != '') {
					...
					if (tag === 'author') {
						...
						shortcode = "[author]\n[author_image]"+imageurl+"[/author_image]\n[author_info]"+content+"[/author_info]\n";
						content = '';
					}
