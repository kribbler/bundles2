...
if ( get_option('woo_compare_product_clean_on_deletion') == 1 ) {
	...
	delete_option( 'woo_compare_product_clean_on_deletion' );
	...
	$wpdb->query("DELETE FROM $wpdb->postmeta WHERE meta_key LIKE '_woo_compare_attribute-%';");
	...
	$wpdb->query('DROP TABLE IF EXISTS ' . $wpdb->prefix . 'woo_compare_attributemeta');
	$wpdb->query('DROP TABLE IF EXISTS ' . $wpdb->prefix . 'woo_compare_categorymeta');
}