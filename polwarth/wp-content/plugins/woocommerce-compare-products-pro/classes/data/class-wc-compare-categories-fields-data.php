...
class WC_Compare_Categories_Fields_Data
{
	...
	public static function get_results($where='', $order='', $limit ='', $output_type='OBJECT') {
		global $wpdb;
		$table_name = $wpdb->prefix . "woo_compare_cat_fields";
		$table_fields = $wpdb->prefix. "woocommerce_attribute_taxonomies";
		if (trim($where) != '')
			$where = " WHERE {$where} ";
		if (trim($order) != '')
		...
		$result = $wpdb->get_results("SELECT * FROM {$table_name} AS cf INNER JOIN {$table_fields} AS f ON(cf.field_id=f.attribute_id) {$where} GROUP BY field_id {$order} {$limit}", $output_type);
		return $result;
	}
	...
	public static function get_catid_results($field_id, $where='', $order='', $limit ='') {
		...
		if (trim($limit) != '')
			$limit = " LIMIT {$limit} ";
		$result = $wpdb->get_col("SELECT DISTINCT cf.cat_id FROM {$table_name} AS cf WHERE field_id='$field_id' {$where} {$order} {$limit}");
		return $result;
	}
	...
	public static function get_fieldid_results($cat_id, $where='', $order='', $limit ='') {
		...
		if (trim($limit) != '')
			$limit = " LIMIT {$limit} ";
		$result = $wpdb->get_col("SELECT DISTINCT cf.field_id FROM {$table_name} AS cf INNER JOIN {$table_fields} AS f ON(cf.field_id=f.attribute_id) WHERE cat_id='$cat_id' {$where} {$order} {$limit}");
		return $result;
	}
